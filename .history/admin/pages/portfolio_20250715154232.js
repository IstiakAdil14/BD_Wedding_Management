...
const PortfolioPage = () => {
  ...
  const fetchEvents = async () => {
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        // Prepend base URL to video if it is a relative path
        const baseUrl = window.location.origin;
        const updatedData = data.map((event) => {
          if (event.video && !event.video.startsWith("http")) {
            return { ...event, video: baseUrl + event.video };
          }
          return event;
        });
  ...
  const handleSaveEvent = async (newEvent) => {
    try {
      let videoUrl = "";
      if (newEvent.video && typeof newEvent.video === "object") {
        // Upload video file first
        const formData = new FormData();
        formData.append("video", newEvent.video);
        ...
        if (uploadRes.ok) {
          ...
          // Prepend server base URL to video URL for frontend access
          const baseUrl = window.location.origin;
      ...
      } else if (typeof newEvent.video === "string") {
        videoUrl = newEvent.video;
      }

      const eventData = { ...newEvent, video: videoUrl };
  ...
  const handleUpdateEvent = async (updatedEvent) => {
    try {
      let videoUrl = "";
      if (updatedEvent.video && typeof updatedEvent.video === "object") {
        // Upload video file first
        const formData = new FormData();
        formData.append("video", updatedEvent.video);
        const uploadRes = await fetch("/api/uploadVideo", {
          method: "POST",
          body: formData,
        });
      ...
      } else if (typeof updatedEvent.video === "string") {
        videoUrl = updatedEvent.video;
      }

      const eventData = { ...updatedEvent, video: videoUrl };
      ...
      <div className="flex flex-1">
        ...
        >
          ...
          <button
            ...
            className={`mt-20 mb-4 ml-4 mr-4 px-4 py-2 rounded hover:bg-green-700 ${
          ...
          {showTable && (
            <div className="overflow-x-auto">
              <table className="min-w-full border-collapse border border-gray-300 mt-4">
                <thead>
                  <tr>
                    <th className="border border-gray-300 px-2 sm:px-4 py-2 text-left min-w-[120px]">
                      Name
                    </th>
                    <th className="border border-gray-300 px-2 sm:px-4 py-2 text-left min-w-[200px]">
                      Description
                    </th>
                    <th className="border border-gray-300 px-2 sm:px-4 py-2 text-left min-w-[120px]">
                      Actions
                ...
                <tbody>
                  {categories.length === 0 ? (
                    <tr>
                      <td
                        colSpan={3}
                        className="border border-gray-300 px-2 sm:px-4 py-2 text-center text-gray-500"
                      >
                    ...
                    categories.map((cat) => (
                      <tr key={cat._id} className="hover:bg-gray-200">
                        ...
                        <td className="border border-gray-300 px-2 sm:px-4 py-2 space-x-2 flex">
                          <button
