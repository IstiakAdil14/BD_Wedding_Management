...
export default function AdminLogin() {
  ...
  useEffect(() => {
    // Clear email and password on mount to prevent autofill
    setEmail("");
    setPassword("");
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        // Store email in localStorage for dashboard profile fetch
  ...
  const handleSave = () => {
    try {
      ...
      alert("Email and password saved locally.");
    } catch (error) {
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-r from-blue-500 via-purple-600 to-pink-500">
      ...
      >
        ...
        <button
          type="button"
          onClick={handleSave}
          className="w-full mt-4 bg-green-600 hover:bg-green-700 text-white font-bold py-3 rounded-lg shadow-lg transition duration-300"
        >
          Save
        </button>
      </form>
