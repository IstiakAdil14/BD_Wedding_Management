...
const PortfolioPage = () => {
  ...
  const handleSaveEvent = async (newEvent) => {
    // After adding, refetch events to refresh the list
    await fetchEvents();
  };
      ...
      <div className="flex flex-1">
        ...
        >
          ...
          <button
            ...
            className={`mt-20 mb-4 ml-4 px-4 py-2 rounded hover:bg-purple-700 ${
              darkMode ? "bg-purple-800 text-white" : "bg-purple-600 text-white"
            }`}
          >
            Events {showEventTable ? <ArrowDropUpIcon /> : <ArrowDropDownIcon />}
          </button>
                ...
                {events.length === 0 ? (
                  ...
                  events.map((event) => (
                    <tr key={event._id} className="hover:bg-gray-200">
                      ...
                      <td className="border border-gray-300 px-4 py-2">{event.image}</td>
                      <td className="border border-gray-300 px-4 py-2">
                        {truncateText(
                          typeof event.description === "string"
                            ? event.description
                            : convertDescriptionToHtml(event.description).replace(
                                /<[^>]+>/g,
                                ""
                              ),
                          20
