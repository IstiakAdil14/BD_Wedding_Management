...
const AdminVerifyOtp = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const response = await axios.post(\`\${process.env.NEXT_PUBLIC_API_BASE_URL}/api/admin/verify-otp\`, {
        email,
        otp,
      });
  ...
  const handleResend = async () => {
    ...
    setLoading(true);
    try {
      const response = await axios.post(\`\${process.env.NEXT_PUBLIC_API_BASE_URL}/api/admin/resend-otp\`, { email });
