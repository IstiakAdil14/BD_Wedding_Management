...
export default function Footer() {
  ...
  const handleSocialLinkChange = (field, value) => {
    ...
    if (value.trim() === "") {
      ...
      setSocialLinksErrors((prev) => ({ ...prev, [field]: "Please enter a valid URL." }));
    } else {
      setSocialLinksErrors((prev) => {
        const newErrors = { ...prev };
        delete newErrors[field];
          ...
          <motion.section className="mb-8" variants={itemVariants}>
              ...
              {Object.entries(socialLinks).map(([key, value]) => (
                <div key={key} className="mb-2">
                  ...
                  {socialLinksErrors[key] && (
                    <p className="mt-1 text-red-600 font-semibold">{socialLinksErrors[key]}</p>
                  )}
                </div>
              ))}
            </div>
            <div className="mt-6">
              <button
                ...
                disabled={loading || phoneError !== "" || emailError !== "" || Object.keys(socialLinksErrors).length > 0}
