...
export default function Footer() {
  ...
  // Validation function for email and Bangladeshi phone number
  const validateContactInfo = (contact) => {
    // Improved email regex to disallow trailing invalid characters
    const emailPattern = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    // Bangladeshi phone number pattern: +8801XXXXXXXXX or 01XXXXXXXXX
    const phonePattern = /^(?:\+8801|01)[3-9]\d{8}$/;

    if (contact.email && !emailPattern.test(contact.email.trim())) {
      return "Invalid email address";
    }
    if (contact.phone && !phonePattern.test(contact.phone.replace(/\s+/g, ""))) {
  ...
  const saveFooterData = async () => {
    ...
    const contactValidationError = validateContactInfo(contactInfo);
