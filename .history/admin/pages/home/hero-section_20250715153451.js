...
export default function HeroSection() {
  ...
  const handleSave = async () => {
    ...
    const heroTitleText = heroTitleEditorState
      .getCurrentContent()
      .getPlainText()
      .trim();
    const heroSubtitleText = heroSubtitleEditorState
      .getCurrentContent()
      .getPlainText()
      .trim();
    ...
    if (!heroTitleText || !heroSubtitleText || !heroCTA.trim()) {
      alert(
        "Please fill in all required fields: Title, Subtitle, and Call to Action Text."
      );
      return;
    }
    try {
      ...
      formData.append(
        "heroTitle",
        JSON.stringify(convertToRaw(heroTitleEditorState.getCurrentContent()))
      );
      formData.append(
        "heroSubtitle",
        JSON.stringify(
          convertToRaw(heroSubtitleEditorState.getCurrentContent())
        )
      );
  ...
  return (
    ...
    >
      <div className="flex flex-1">
        {/* Sidebar */}
        {isMobile ? (
          <div className="p-4 sticky mr-5 top-0 left-0 z-50">
            <HamburgerMenu />
          </div>
        ) : (
          <motion.nav
