...
export default function Contact() {
  ...
  const [contactInfo, setContactInfo] = useState({
    phoneNumbers: [],
    email: "",
    // Change businessHours to structured object
    businessHours: {
      openAt: "09:00 AM",
      closeAt: "06:00 PM",
      days: {
...
=======
  // Remove old isBusinessHoursValid state and validation function since UI changed
  // Add helper functions for time options and day toggling
    ...
    setContactInfo((prev) => ({
      ...
      businessHours: {
        ...
        days: {
          ...
          [day]: !prev.businessHours.days[day],
        },
      },
    }));
  };
=======
            ...
            <div className="mb-4">
              ...
              <div className="flex space-x-4 items-center">
                <div>
                  <label className="block mb-1">Open At</label>
                  <select
                    value={contactInfo.businessHours.openAt}
                    onChange={(e) => handleBusinessHoursChange("openAt", e.target.value)}
                ...
                <div>
                  <label className="block mb-1">Close At</label>
                  <select
                    value={contactInfo.businessHours.closeAt}
                    onChange={(e) => handleBusinessHoursChange("closeAt", e.target.value)}
                    className={`rounded border p-2 ${
                      darkMode
                        ? "border-gray-600 bg-gray-700 text-gray-100"
                        : "border-gray-300 bg-white text-gray-900"
              ...
              <div className="mt-2 flex space-x-2">
                {Object.keys(contactInfo.businessHours.days).map((day) => (
                  <label key={day} className="inline-flex items-center space-x-1 cursor-pointer">
                    <input
                      type="checkbox"
                      checked={contactInfo.businessHours.days[day]}
                      onChange={() => toggleDay(day)}
                      className="rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                    />
                    <span className={`${darkMode ? "text-gray-100" : "text-gray-900"}`}>{day}</span>
                  </label>
                ))}
              </div>
            </div>
  ...
  // New state for business hours validity meter
  const [isBusinessHoursValid, setIsBusinessHoursValid] = useState(true);
