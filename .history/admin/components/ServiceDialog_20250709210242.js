...
import { FaPaintBrush, FaUtensils, FaCamera, FaMusic, FaUndo, FaRedo, FaLink, FaImage, FaAlignLeft, FaAlignCenter, FaAlignRight, FaAlignJustify } from "react-icons/fa";
import {
  Editor,
  EditorState,
  convertFromRaw,
...
const FONT_FAMILIES = [
  { label: "Arial", style: "font-family-arial", title: "Arial" },
  { label: "Georgia", style: "font-family-georgia", title: "Georgia" },
  { label: "Times New Roman", style: "font-family-times", title: "Times New Roman" },
  { label: "Courier New", style: "font-family-courier", title: "Courier New" },
];

const FONT_SIZES = [
  { label: "12", style: "font-size-12", title: "12px" },
  { label: "14", style: "font-size-14", title: "14px" },
  { label: "16", style: "font-size-16", title: "16px" },
  { label: "18", style: "font-size-18", title: "18px" },
  { label: "24", style: "font-size-24", title: "24px" },
  { label: "32", style: "font-size-32", title: "32px" },
];

const ALIGNMENT_TYPES = [
  { label: <FaAlignLeft />, style: "align-left", title: "Align Left" },
  { label: <FaAlignCenter />, style: "align-center", title: "Align Center" },
  { label: <FaAlignRight />, style: "align-right", title: "Align Right" },
  { label: <FaAlignJustify />, style: "align-justify", title: "Justify" },
];
...
function StyleButton({ onToggle, active, label, style, title }) {
  const className = `px-2 py-1 border rounded mr-1 cursor-pointer select-none ${
...
function BlockStyleButton({ onToggle, active, label, style, title }) {
  const className = `px-2 py-1 border rounded mr-1 cursor-pointer select-none ${
    active ? "bg-blue-600 text-white" : "bg-white text-gray-700"
  }`;
  return (
...
function FontFamilyDropdown({ editorState, onToggle }) {
  const currentStyle = editorState.getCurrentInlineStyle();
  return (
    <select
      title="Font Family"
      className="border rounded mr-1 p-1 cursor-pointer"
      value={
        FONT_FAMILIES.find((f) => currentStyle.has(f.style))?.style || ""
      }
      onChange={(e) => {
        e.preventDefault();
    ...
    >
      ...
      {FONT_FAMILIES.map(({ label, style, title }) => (
        <option key={style} value={style} title={title}>
          {label}
        </option>
      ))}
    </select>
...
function FontSizeDropdown({ editorState, onToggle }) {
  const currentStyle = editorState.getCurrentInlineStyle();
  return (
    <select
      title="Font Size"
      className="border rounded mr-1 p-1 cursor-pointer"
      value={FONT_SIZES.find((f) => currentStyle.has(f.style))?.style || ""}
...
export default function ServiceDialog({
  ...
  useEffect(() => {
    if (initialData) {
      ...
      // Initialize descriptionEditorState from initialData.description
      try {
        const contentState = initialData.description
          ? convertFromRaw(JSON.parse(initialData.description))
          : null;
        if (contentState) {
  ...
  const toggleInlineStyle = (inlineStyle) => {
    setDescriptionEditorState(
      RichUtils.toggleInlineStyle(descriptionEditorState, inlineStyle)
    );
  };

  const toggleBlockType = (blockType) => {
    setDescriptionEditorState(
      RichUtils.toggleBlockType(descriptionEditorState, blockType)
    );
  };

  const handleUndo = () => {
    setDescriptionEditorState(EditorState.undo(descriptionEditorState));
  };

  const handleRedo = () => {
    setDescriptionEditorState(EditorState.redo(descriptionEditorState));
  };
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg p-6 max-w-lg w-full shadow-lg overflow-auto max-h-[90vh] relative">
        ...
        >
          ...
          <div>
            ...
            />
            <div
              className="w-full p-2 border border-gray-300 rounded bg-white text-gray-900 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-100 min-h-[100px]"
              style={{ cursor: "text" }}
              onClick={() => {
          ...
          <div>
            <label className="block font-semibold mb-1">Icon URL</label>
            <input
              type="text"
              name="iconName"
              value={serviceData.iconName}
              onChange={handleChange}
