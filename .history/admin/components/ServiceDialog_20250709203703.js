...
export default function ServiceDialog({
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    // Convert EditorState to raw JSON string and update serviceData.description
    const contentState = descriptionEditorState.getCurrentContent();
    const raw = JSON.stringify(convertToRaw(contentState));
    onSave({ ...serviceData, description: raw });
  };

  const iconMap = {
    FaPaintBrush: (
      <FaPaintBrush className="w-8 h-8 text-pink-500 cursor-pointer" />
    ),
    FaUtensils: <FaUtensils className="w-8 h-8 text-pink-500 cursor-pointer" />,
    FaCamera: <FaCamera className="w-8 h-8 text-pink-500 cursor-pointer" />,
    FaMusic: <FaMusic className="w-8 h-8 text-pink-500 cursor-pointer" />,
  };
