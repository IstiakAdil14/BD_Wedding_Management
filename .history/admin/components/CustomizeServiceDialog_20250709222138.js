...
const CustomizeServiceDialog = ({ isOpen, onClose, onSave, service }) => {
  ...
  const onEditorStateChange = (editorState) => {
    ...
    } else if (editorState && editorState.editorState) {
      // react-draft-wysiwyg sometimes passes an object with editorState property
      const actualEditorState = editorState.editorState;
      setEditorState(actualEditorState);
      const contentState = actualEditorState.getCurrentContent();
      const raw = convertToRaw(contentState);
      setFormData((prev) => ({
        ...prev,
        categoryDescription: JSON.stringify(raw),
