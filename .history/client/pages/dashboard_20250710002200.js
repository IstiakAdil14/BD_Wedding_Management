...
export default function ClientDashboard() {
  ...
  const downloadSinglePdf = (booking) => {
    ...
    const addLine = (label, value) => {
      ...
      if (label === "Payment Account Number" && value && value.length > 10) {
        // Split long payment account number into two lines
        const firstLine = value.substring(0, 10);
        const secondLine = value.substring(10);
        doc.text(firstLine, 80, y);
        y += lineHeight;
        doc.text(secondLine, 80, y);
        y += lineHeight;
      } else {
        const xPos = label === "Payment Account Number" ? 80 : 60;
    ...
    try {
      if (booking.specialRequests) {
        const contentState = convertFromRaw(JSON.parse(booking.specialRequests));
    ...
    addLine("Event Date", booking.eventDate ? new Date(booking.eventDate).toLocaleDateString() : "-");
    addLine("Special Requests", specialRequestsText);
    addLine("Payment Method", booking.paymentMethod || "-");
    addLine("Payment Account Number", booking.paymentAccountNumber || "-");
    addLine("Status", booking.status || "-");
    addLine("Created At", booking.createdAt ? new Date(booking.createdAt).toLocaleString() : "-");
