...
const PackageDetails = () => {
  ...
  useEffect(() => {
    const fetchUserProfile = async () => {
      try {
        ...
        const res = await fetch(`http://localhost:5000/api/auth/client-personal-details?email=${encodeURIComponent(authEmail)}`);
        if (!res.ok) {
        ...
        if (profileData.fullName) {
          setBookingDetails((prev) => ({ ...prev, name: profileData.fullName }));
        }
        if (profileData.phoneNumber) {
          setBookingDetails((prev) => ({ ...prev, phone: profileData.phoneNumber }));
        }
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:5000/api/services/${id}/book`, {
        method: 'POST',
        headers: {
