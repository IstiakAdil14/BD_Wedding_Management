...
const PackageDetails = () => {
  ...
  useEffect(() => {
    if (bookingDetails.specialRequests && typeof bookingDetails.specialRequests === "string") {
      try {
        const contentState = convertFromRaw(JSON.parse(bookingDetails.specialRequests));
        setBookingDetails((prev) => ({
          ...prev,
          specialRequestsEditorState: EditorState.createWithContent(contentState),
        }));
      } catch {
        setBookingDetails((prev) => ({
          ...prev,
  ...
  const handleSpecialRequestsChange = (editorState) => {
    setBookingDetails((prev) => ({ ...prev, specialRequestsEditorState: editorState }));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const specialRequestsRaw = JSON.stringify(
        convertToRaw(bookingDetails.specialRequestsEditorState.getCurrentContent())
      );
  ...
  return (
    ...
    >
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          <label htmlFor="email" className="block font-semibold mb-1 text-black dark:text-white">
            Email
          </label>
          {isAuthenticated ? (
            <input
        ...
        <div>
          <label htmlFor="phone" className="block font-semibold mb-1 text-black dark:text-white">
            Phone
          </label>
          <input
            type="tel"
        ...
        <div>
          <label htmlFor="eventDate" className="block font-semibold mb-1 text-black dark:text-white">
            Event Date
          </label>
          <input
            type="date"
        ...
        <div>
          <label htmlFor="specialRequests" className="block font-semibold mb-1 text-black dark:text-white">
            Special Requests
          </label>
        <div
          style={{
