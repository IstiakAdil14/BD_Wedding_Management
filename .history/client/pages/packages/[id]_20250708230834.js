...
const PackageDetails = () => {
  ...
  const handleChange = (e) => {
    ...
    setBookingDetails((prev) => ({
      ...prev,
      [name]: value,
      paymentAccountNumberError: name === "paymentAccountNumber" ? "" : prev.paymentAccountNumberError,
    }));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (paymentMethod === "nogod" || paymentMethod === "bkash") {
      if (!bdPhoneRegex.test(paymentAccountNumber)) {
        setBookingDetails((prev) => ({
          ...prev,
          paymentAccountNumberError: "Please enter a valid Bangladeshi phone number for the selected payment method.",
        }));
        return;
      }
    } else {
      if (accountNumbers[paymentMethod] !== paymentAccountNumber) {
        setBookingDetails((prev) => ({
          ...prev,
          paymentAccountNumberError: "The account number does not match the selected payment method's account number.",
        }));
        return;
      }
    }
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          <label
            ...
            className="block font-semibold mb-1 text-black dark:text-white"
          >
            {bookingDetails.paymentMethod === "nogod" || bookingDetails.paymentMethod === "bkash"
              ? "Number"
              : "Account Number"}
          </label>
          <input
            ...
            required
            className={`w-full p-2 border rounded bg-white dark:bg-gray-800 dark:text-white ${
              bookingDetails.paymentAccountNumberError ? "border-red-500" : "border-gray-300 dark:border-gray-600"
            }`}
            placeholder={
              bookingDetails.paymentMethod === "nogod" || bookingDetails.paymentMethod === "bkash"
                ? "Enter your number"
          ...
          {bookingDetails.paymentAccountNumberError && (
            <p className="mt-1 text-sm text-red-600">{bookingDetails.paymentAccountNumberError}</p>
          )}
        </div>
        <div className="mb-6">
          <p
