...
const Testimonials = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch("http://localhost:5000/api/testimonials", {
        ...
        body: JSON.stringify({
          clientName: profile?.fullName || user?.name || user?.email || "Anonymous",
          email: profile?.email || user?.email || null,
          clientImage: profile?.profilePicture || user?.photoURL || "/default-profile.png",
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-100 flex flex-col px-4 sm:px-8">
      <motion.main
        className="flex-grow max-w-full md:max-w-5xl lg:max-w-4xl xl:max-w-3xl mx-auto"
      ...
      >
        ...
        >
          {testimonials.map((testimonial) => (
            <motion.div
              key={testimonial._id}
              className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6 flex flex-col items-center text-center w-[calc((100vw-128px)/5)] min-w-[300px] flex-shrink-0 cursor-grab mt-4"
            ...
            >
              ...
              >
                <div className="flex items-center space-x-4 mb-4">
                  <img
                    src={profile?.profilePicture || user?.photoURL || "/default-profile.png"}
                    alt={profile?.fullName || user?.name || user?.email || "User"}
                    className="w-16 h-16 rounded-full object-cover"
                  />
                  <div>
                    <p className="font-semibold">{profile?.fullName || user?.name || user?.email || "User"}</p>
                  </div>
                </div>
                <div>
