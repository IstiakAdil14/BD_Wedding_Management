    ...
    try {
      ...
      try {
        ...
        if (!loginRes.ok) {
          let loginErrorText;
          try {
            const loginErrorData = await loginRes.clone().json();
            loginErrorText = loginErrorData.message || JSON.stringify(loginErrorData);
          } catch {
            try {
              loginErrorText = await loginRes.clone().text();
            } catch {
              loginErrorText = "Failed to log in after registration";
            }
          }
          throw new Error(loginErrorText || "Failed to log in after registration");
        }
        ...
        if (loginData.accessToken) {
          ...
          // Optionally, you can update any auth context or state here if needed
        }
        setSuccess("Personal details saved and logged in successfully");
        setTimeout(() => {
          router.push("/");
        }, 2000);
      } catch (loginErr) {
        setError(loginErr.message);
        // Removed redirect to verify-otp page to avoid sending client/pages/verify-otp.js
        // Optionally, you can handle login failure here without redirecting
        // For example, just show the error message and stay on the same page
      }
    } catch (err) {
