...
export default function ClientDashboard() {
  ...
  const downloadSinglePdf = (booking) => {
    ...
    const addLine = (label, value) => {
      ...
      if (label === "Payment Account Number" && value) {
        // Put the text "Number" one line below the label "Payment Account"
        const parts = label.split(" ");
        if (parts.length > 1) {
          doc.text(parts[0] + " " + parts[1], 14, y);
          y += lineHeight;
          doc.text(parts.slice(2).join(" ") + " " + value, 14, y);
          y += lineHeight;
        } else {
          y += lineHeight;
          doc.text(value || "-", 14, y);
