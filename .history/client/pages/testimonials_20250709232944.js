...
const Testimonials = () => {
  ...
  // Image insertion handler (basic URL prompt)
  const insertImage = () => {
  ...
  // StyleButton component updated to handle dropdowns
  const StyleButton = ({ onToggle, style, active, label, isDropdown, options, onSelect }) => {
    if (isDropdown) {
      return (
        <select
              ...
              >
                ...
                <div>
                  ...
                  <div className="mb-2 flex flex-wrap items-center space-x-1">
                    ...
                    <StyleButton
                      isDropdown={true}
                      style={currentStyle.find((s) => FONT_FAMILIES.some(f => f.style === s)) || FONT_FAMILIES[0].style}
                      options={FONT_FAMILIES}
                      onSelect={toggleFontFamily}
                    />
                    {/* Font size selector */}
                    <StyleButton
                      isDropdown={true}
                      style={currentStyle.find((s) => FONT_SIZES.some(f => f.style === s)) || FONT_SIZES[0].style}
                      options={FONT_SIZES}
                      onSelect={toggleFontSize}
                    />
                    {/* Link controls */}
                    <button
                      type="button"
