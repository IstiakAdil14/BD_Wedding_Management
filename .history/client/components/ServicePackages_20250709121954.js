  ...
  const handleSubmit = async (e) => {
    ...
    if (paymentMethod === "nogod" || paymentMethod === "bkash") {
      if (!bdPhoneRegex.test(paymentAccountNumber)) {
        alert("Please enter a valid Bangladeshi phone number for the selected payment method.");
        return;
      }
    } else {
      if (accountNumbers[paymentMethod] !== paymentAccountNumber) {
        alert("The account number does not match the selected payment method's account number.");
        return;
      }
    }
          ...
          >
            ...
            >
              ...
              <div>
                ...
                >
                  {customDetails.paymentMethod === "nogod" || customDetails.paymentMethod === "bkash"
                    ? "Number"
                    : "Account Number"}
                </label>
                <input
                  ...
                  className={`w-full p-2 border rounded bg-gray-100 dark:bg-gray-700 dark:text-white ${
                    customDetails.paymentAccountNumberError ? "border-red-500" : "border-gray-300 dark:border-gray-600"
                  }`}
                  placeholder={
                    customDetails.paymentMethod === "nogod" || customDetails.paymentMethod === "bkash"
                      ? "Enter your number"
                ...
                {customDetails.paymentAccountNumberError && (
                  <p className="mt-1 text-sm text-red-600">{customDetails.paymentAccountNumberError}</p>
                )}
              </div>
              <div className="mb-6">
                <p
