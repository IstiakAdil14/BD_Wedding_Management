...
const CustomizeServiceDialog = ({ isOpen, onClose, onSave }) => {
  ...
  const fetchEventTypesByCategory = async (category) => {
    try {
      ...
      const res = await fetch(
        `/api/customizeServices/eventTypesByCategory/${encodeURIComponent(
          category
        )}`
      );
      if (res.ok) {
        ...
        const mappedEventTypes = data.map((et, index) => ({
          _id: index,
          name: et,
        }));
        ...
        console.error(
          "Failed to fetch event types by category, status:",
          res.status
        );
        setEventTypes([]);
      }
  ...
  const handleChange = (e) => {
    ...
    if (name === "category") {
      ...
      updatedData.categoryDescription = selectedCategory
