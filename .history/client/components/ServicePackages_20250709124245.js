...
const ServicePackages = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (paymentMethod === "nogod" || paymentMethod === "bkash") {
      ...
      // Basic validation: check if paymentAccountNumber matches card number pattern (digits and spaces, length 13-19)
      const cardNumberPattern = /^[0-9 ]{13,19}$/;
      if (!paymentAccountNumber || !cardNumberPattern.test(paymentAccountNumber) || /^0+$/.test(paymentAccountNumber.replace(/\s/g, ''))) {
        alert(
