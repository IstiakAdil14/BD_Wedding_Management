...
export default function ClientPersonalDetailsForm({ email }) {
  ...
  useEffect(() => {
    ...
    const objectUrl = URL.createObjectURL(details.profilePicture);
    setPreviewUrl(objectUrl);

    // Clean up the object URL when component unmounts or profilePicture changes
    return () => {
      if (objectUrl) {
    ...
    try {
      ...
      if (details.profilePicture && details.profilePicture instanceof File) {
        ...
        if (!uploadRes.ok) {
          let errorText;
          try {
            const errorData = await uploadRes.clone().json();
            errorText = errorData.error || JSON.stringify(errorData);
          } catch {
            try {
              errorText = await uploadRes.clone().text();
            } catch {
